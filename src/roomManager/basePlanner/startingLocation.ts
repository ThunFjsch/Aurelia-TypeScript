...
export function getLowestScoreDTMap(scoreMap: number[][], topN = settings.buildPlanner.maxSelection) {
...
export function pickStartingLocations(dt: number[][], room: Room): number[][] {
    ...
    for (let y = 0; y < 50; y++) {
        for (let x = 0; x < 50; x++) {
            ...
            const score = getStartingLocationScore(
    ...
    const cutoffIndex = Math.floor(allScores.length * percentile);
...
export function getStartingLocationScore(controller: RoomPosition, minDistanceFromController: number, sources: Source[], minerals: Mineral[], x: number, y: number, dt: number[][]): number {
